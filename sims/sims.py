...
import numpy as np
import io, sys, os, re, inspect, datetime, warnings, json
...
class SIMSBase(object):
    ...
    def __init__(self, fileobject):
        """ This class object does not open or close files, and nothing will
            be read by default. An empty header will be created and all rele- 
            vant methods for reading the header and data are available.
        """
        self.fh = fileobject
        self.header = {}
    ...
    def peek(self):
        ...
        if self.header['file type'] not in supported_file_types:
            raise NotImplementedError("File of type {} is not supported at the moment.".format(self.header['file type']))
    ...
    def read_header(self):
        ...
        if polylist_pos < 0:
            warnings.warn('Beginning of PolyList not found, don\'t know where nanoSIMS header starts.')
        else:
            ...
            if champslist_pos >= 0:
                ...
                self.header['champslist length'] = unpack(self.header['byte order'] + 'i', hdr.read(4))[0]
            ...
            if offsetlist_pos >= 0:
                ...
                self.header['offsetlist length'] = unpack(self.header['byte order'] + 'i', hdr.read(4))[0]
        ...
        if analparamnano_pos < 0:
            warnings.warn('Anal_param_nano not found in header, don\'t know where PrimaryBeam section starts.')
        else:
            ...
            self.header['SecondaryBeam']['pressure multicollection chamber'] = self.header['Detectors'].pop('pressure multicollection chamber')
            
            # Header for non-nano SIMS
            magic = unpack(self.header['byte order'] + 'i', hdr.read(4))[0]
            if magic != 2306:
                raise ValueError('SIMSHeader magic number not found here at byte {}.'.format(hdr.tell()-4))
            ...
            if self.header['analysis version'] >= 5:
                if analparamnanobis_pos < 0:
                    warnings.warn('Anal_param_nano_bis not found in header, don\'t know where second Detectors section starts.')
                else:
    ...
    def _main_header(self, hdr):
        ...
        if self.header['file type'] in (21, 22, 41): hdr.seek(4, 1) # 4 bytes unused
    ...
    def _nanosims_header(self, hdr):
        ...
        if b_field_index < 0: b_field_index = None
    ...
    def _sib_center(self, hdr):
        ...
        if d['detector'] < 0: d['detector'] = None
    ...
    def _energy_center(self, hdr):
        ...
        d['count time'] /= 100 # 10 ms increments to seconds
        if d['detector'] < 0: d['detector'] = None
        if d['b field index'] < 0: d['b field index'] = None
        return d
    ...
    def _trolley(self, hdr):
        ...
        d = {}
        d['mass label'], d['mass'], d['radius'], d['deflection plate 1'], d['deflection plate 2'], \
            d['detector'], d['exit slit'], d['real trolley'], \
    ...
    def _detectors1(self, hdr):
        ...
        d['pressure multicollection chamber'] = self._cleanup_string(d['pressure multicollection chamber'])
    ...
    def _exit_slits(self, hdr):
        ...
        d['exit slit size label'] = exit_slit_size_labels.get(d['exit slit size'], str(d['exit slit size']))
    ...
    def _presets(self, hdr, group=None):
        ...
        for p in range(d['parameters']):
            ...
            if not param_name: param_name = str(param_id)
            d[param_name] = value
    ...
    def _cleanup_date(self, date):
        ...
        """
        if not (date and isinstance(date, str)): return None
    ...
    def read_limage_lut(self, *names, smooth=True):
        ...
        for l in range(n_luts):
            ...
            if (not name.startswith('?')) and ((name in names) or (len(names) == 0)):
                ...
                lut_data = np.fromstring(data[offset + l*768 : offset + (l+1)*768], dtype='B').reshape(3, 256).T
                lut_data = norm(lut_data)
    ...
    def __init__(self, filename, load_luts=False):
        ...
        if isinstance(filename, str):
            if filename.lower().endswith('.gz'):
                ...
                if not lzma:
                    raise NotImplementedError("LZMA module is not installed on your system. Use Python 3.3 or higher, or install a module to handle lzma/xz compressed files.")
                self.fh = lzma.LZMAFile(filename, mode='rb')
            elif filename.lower().endswith('.zip'):
                raise NotImplementedError("There are too many problems with zip compression support. Use gzip, bzip2, or xz compression instead.")
                z = zipfile.ZipFile(filename, mode='r')
                # zipfile can store multiple files, pick first from list, raise warning if more than one
                namelist = z.namelist()
                if len(namelist) > 1:
                ...
                # zipfile does not support seek and tell; read entire file, create BytesIO instead
        ...
        if load_luts: SIMSLut.__init__(self)
