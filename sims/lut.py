""" Convert and load Cameca and L'image look-up tables.
    ...
    files and the L'image colour table file. Loaded LUTs are converted to
    Matplotlib Colormaps, and registered with Matplotlib. Use mpl.colormaps()
    to see all registered Colormaps and mpl.set_cmap() to set the Colormap
    to use.
...
def load_cameca_lut(*names, smooth=True):
    """ Load a Cameca Look-Up Table.
        ...
        'cameca ' prefix may be omitted. If no name is given, all LUTs in the lut-
...
def load_limage_lut(*names, smooth=True):
    """ Load a L'image Look-Up Table.
        ...
        the 'limage ' prefix may be omitted. If no name is given, all LUTs in the
        file will be loaded.
